...
//			sql += " and m.name like '%" + uname + "%'";
    ...
    public Page getInviteList(Integer page, Integer pageSize, Integer agentStatus, int memberId) {
        if (agentStatus == null) {
            return this.daoSupport.queryForPage("SELECT m.* FROM es_member m WHERE m.parentid = ? ORDER BY m.member_id ASC",
                    page, pageSize, memberId);
        } else {
            return this.daoSupport.queryForPage("SELECT m.* FROM es_member m WHERE m.parentid = ? and agent_status = ? ORDER BY m.member_id ASC",
                    page, pageSize, memberId, agentStatus);
        }
    }

    @Override
    public int inviteReview(Integer reviewerId, Integer memberId, boolean pass, String reason) {
        Member member = this.get(memberId);
        member.setAgent_status(pass ? 1 : 2);
        member.setReview_time(DateUtil.getDateline());
        member.setAgent_remark(reason);
        this.daoSupport.update("es_member", member, " member_id = " + member.getMember_id()
                + " and parentid = " + reviewerId);
        return 0;
    }
